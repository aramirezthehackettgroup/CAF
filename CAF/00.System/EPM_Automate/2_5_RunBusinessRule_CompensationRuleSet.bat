...
REM Author: Operez
REM Date: Nov 2016
REM 2_3_RunBusinessRule_CompensationRuleSet.bat 
REM ---------------------------------------------------------
...
SET message="%date% %time% - 2_3_RunBusinessRule_CompensationRuleSet"
ECHO %message:"=%
ECHO %message:"=% >> %log_file%
CALL %epmautomate_client% RunBusinessRule %Rulename1% EntityPrompt=%EntityPrompt% VersionPrompt=%VersionPrompt% >> %log_file%
CALL %epmautomate_client% RunBusinessRule %Rulename2% EntityPrompt=%EntityPrompt% VersionPrompt=%VersionPrompt% >> %log_file%
CALL %epmautomate_client% RunBusinessRule %Rulename3% EntityPrompt=%EntityPrompt% VersionPrompt=%VersionPrompt% ProjectPrompt=%ProjectPrompt% >> %log_file%
CALL %epmautomate_client% RunBusinessRule %Rulename4% ProjectPrompt=%ProjectPrompt% VersionPrompt=%VersionPrompt% >> %log_file%
IF %ERRORLEVEL% NEQ 0 ( 
CALL :ErrorPara %message% %log_file% 
)
ECHO.
