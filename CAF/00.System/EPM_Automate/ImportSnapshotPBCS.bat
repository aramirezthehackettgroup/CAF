...
ECHO %message:"=%
ECHO %message:"=% >> %log_file%
CALL %epmautomate_client% deletefile %snapshotname% >> %log_file%
IF %ERRORLEVEL% EQU 0 (
      GOTO:PROCESS
...
ECHO %message:"=%
ECHO %message:"=% >> %log_file%
CALL %epmautomate_client% uploadfile %sourcefiledir%\%snapshotname% >> %log_file%
IF %ERRORLEVEL% NEQ 0 ( 
CALL :ErrorPara %message% %log_file% 
EXIT )
ECHO.
...
SET message="%date% %time% - Import Snapshot %snapshotname%"
ECHO %message:"=%
ECHO %message:"=% >> %log_file%
CALL %epmautomate_client% importsnapshot %snapshotname% >> %log_file%
IF %ERRORLEVEL% NEQ 0 ( 
CALL :ErrorPara %message% %log_file% 
)
ECHO.
...
ECHO %message:"=%
ECHO %message:"=% >> %log_file%
MOVE %sourcefiledir%\%snapshotname% %backupfiledir% >> %log_file%
IF %ERRORLEVEL% NEQ 0 ( 
CALL :ErrorPara %message% %log_file% 
EXIT )
ECHO.
...
ECHO %message:"=%
ECHO %message:"=% >> %log_file%
FOR /f "tokens=1,2 delims=." %%a IN ("%snapshotname%") do (
SET file_base=%%a
SET extn=%%b
)
...
REN %backupfiledir%\%snapshotname% %file_base%_%timestamp%.%extn% >> %log_file%
