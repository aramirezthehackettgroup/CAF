...
SET message="%date% %time% - 3_1_RunBusinessRule_PNLPlanClearData"
ECHO %message:"=%
ECHO %message:"=% >> %log_file%
CALL %epmautomate_client% RunBusinessRule %Rulename% EntityPrompt=%EntityPrompt% ScenarioPrompt=%ScenarioPrompt% ProjectPrompt=%ProjectPrompt% VersionPrompt=%VersionPrompt% MultiYearPrompt=%MultiYearPrompt% >> %log_file%
IF %ERRORLEVEL% NEQ 0 ( 
CALL :ErrorPara %message% %log_file% 
)
ECHO.
