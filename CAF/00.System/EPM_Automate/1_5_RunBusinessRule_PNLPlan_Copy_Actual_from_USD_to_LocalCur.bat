...
REM Author: EVAZQUEZ
REM Date: Jan 2017
REM 1_5_RunBusinessRule_PNLPlan_Copy_Actual_from_USD_to_LocalCur.bat 
REM ---------------------------------------------------------
...
SET message="%date% %time% - 1_5_PNLPlan_Copy_Actual_from_USD_to_LocalCur"
ECHO %message:"=%
ECHO %message:"=% >> %log_file%
CALL %epmautomate_client% RunBusinessRule %Rulename% YearPrompt=%YearPrompt% >> %log_file%
IF %ERRORLEVEL% NEQ 0 ( 
CALL :ErrorPara %message% %log_file% 
)
ECHO.
