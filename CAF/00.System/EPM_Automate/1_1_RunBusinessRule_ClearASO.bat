...
REM Author: Operez
REM Date: Nov 2016
REM 1_1_RunBusinessRule_ClearASO.bat 
REM ---------------------------------------------------------
...
SET message="%date% %time% - 1_1_RunBusinessRule_ClearASO"
ECHO %message:"=%
ECHO %message:"=% >> %log_file%
CALL %epmautomate_client% RunBusinessRule %Rulename% ScenarioPrompt=%ScenarioPrompt% VersionPrompt=%VersionPrompt% MultiYearPrompt=%MultiYearPrompt% DataTypePrompt=%DataTypePrompt% >> %log_file%
IF %ERRORLEVEL% NEQ 0 ( 
CALL :ErrorPara %message% %log_file% 
)
ECHO.
