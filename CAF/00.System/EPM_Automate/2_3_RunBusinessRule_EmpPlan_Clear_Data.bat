...
REM Author: Operez
REM Last Modified by: EVAZQUEZ
REM Date: JAN 2017
REM 2_3_RunBusinessRule_EmpPlan_Clear_Data.bat 
REM ---------------------------------------------------------
...
SET message="%date% %time% - 2_3_RunBusinessRule_EmpPlan_Clear_Data"
ECHO %message:"=%
ECHO %message:"=% >> %log_file%
CALL %epmautomate_client% RunBusinessRule %Rulename% ScenarioPrompt=%ScenarioPrompt% EntityPrompt=%EntityPrompt% VersionPrompt=%VersionPrompt% MultiDataTypePrompt=%MultiDataTypePrompt% MultiYearPrompt=%MultiYearPrompt%>> %log_file%
IF %ERRORLEVEL% NEQ 0 ( 
CALL :ErrorPara %message% %log_file% 
)
ECHO.
